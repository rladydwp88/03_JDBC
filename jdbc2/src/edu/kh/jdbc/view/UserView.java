...
public class UserView {
	...
	private void multiInsertUser() throws Exception {
		...
		for(int i = 0; i < input; i++) {
			String userId = null; // 입력 된 아이디를 저장할 변수
		...
		// -> 결과로 삽입된 행의 개수 반환
		int result = service.multiInsetUser(userList);
	...
	private void updateName() throws Exception {
		...
		// 위에서 조회된 회원(userNo)의 이름 수정 서비스 호출(UPDATE) 후 결과 반환(int) 받기
	...
	private void selectAll() throws Exception {
		...
		// 조회 결과가 없을 경우
		if(userList.isEmpty()) { // userList 비어있다면
			System.out.println("\n*** 조회 결과가 없습니다***\n");
			return;
		}
