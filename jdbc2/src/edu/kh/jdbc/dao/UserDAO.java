...
// 클래스명.메서드()이 아닌 메서드명() 만 작성해도 호출 가능하게 함.
...
public class UserDAO {
	...
	public int insertUser(Connection conn, User user) throws Exception {
		...
		try {
			...
			// 5. SQL(INSERT) 수행(executeUpdate()) 후 결과(삽입 된 행의 갯수) 반환 받기
	...
	public List<User> selectAll(Connection conn) throws Exception {
		...
		try {
			...
			// 5. SQL(SELECT) 수행(executeQuary) 후 결과 반환(ResultSet) 받기
			rs = pstmt.executeQuery();
	...
	/** 6. ID, PW가 일치하는 회원의 USER_NO 조회 DAO
	 * @param conn
	 * @param userId
	 * @param userPw
	 * @return userNo
